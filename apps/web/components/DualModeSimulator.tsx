        ...
        <div className="overflow-hidden rounded-[28px] bg-[#f4f1e8]">
          ...
          <div className="px-5 pb-8 pt-5">
            ...
            {isStudent ? <motion.div key="student" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div className="mt-3 rounded-2xl bg-[#d8f36d] p-4"><div className="flex items-center gap-2 font-mono text-[9px] font-bold uppercase"><Sparkles size={13} /> Trợ lý AI</div><p className="mt-2 text-xs font-extrabold">Tạo quiz 10 câu trong 30 giây</p><button type="button" onClick={() => window.dispatchEvent(new CustomEvent('classsync:toast', { detail: { type: 'info', title: 'Tạo quiz demo', message: 'AI đang tạo đề mẫu trong môi trường demo.' } }))} className="mt-3 flex items-center gap-1 text-[10px] font-bold underline underline-offset-2">Bắt đầu <ChevronRight size={13} /></button></div>
            </motion.div>}
